...
const Pipeline: React.FC = () => {
  ...
  const handleProcessSignal = async () => {
    ...
    try {
      await pipelineService.processSignal(selectedSource, signalInput);
              ...
              {glassCard(
                <>
                  ...
                  <Stack direction="row" spacing={0.5} sx={{ mb: 2 }}>
                    {(['x', 'telegram', 'reddit', 'discord'] as SocialPlatform[]).map((platform) => {
                  ...
                  <Button
                    ...
                    startIcon={isProcessing ? <SpeedIcon /> : <SendIcon />}
                    sx={{
                      background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                  ...
                  {isProcessing && <LinearProgress sx={{ mt: 2, borderRadius: 1, height: 3 }} color="success" />}
                    ...
                    <Box sx={{ mt: 3 }}>
                      ...
                      <Stack spacing={1.5}>
                        {recentSignals.slice(0, 3).map((signal) => (
                          ...
                          >
                            <Typography variant="body2" sx={{ fontWeight: 600, color: theme.palette.text.primary, mb: 1, fontSize: '0.8rem', lineHeight: 1.4 }}>
                              {signal.hypothesis.slice(0, 120)}...
                            </Typography>

                            {/* Discovered assets with LINKS */}
                            <Stack direction="row" spacing={0.5} flexWrap="wrap" sx={{ mb: 1, gap: 0.5 }}>
                              {signal.discoveredAssets.slice(0, 5).map((asset: DiscoveredAsset, idx: number) => (
                                <Tooltip key={idx} title={`Open ${asset.platform} → ${asset.symbol}`} arrow>
                                  <Chip
                                    label={`${asset.symbol} · ${asset.assetClass}`}
                                    size="small"
                                    onClick={() => window.open(asset.platformUrl, '_blank')}
                                    icon={<OpenInNewIcon sx={{ fontSize: '12px !important' }} />}
                                    sx={{
                                      fontSize: '0.62rem', height: 22, fontWeight: 700, cursor: 'pointer',
                                      background: `${categoryColors[asset.assetClass] || '#6366f1'}12`,
                                      color: categoryColors[asset.assetClass] || '#6366f1',
                                      border: `1px solid ${categoryColors[asset.assetClass] || '#6366f1'}25`,
                                      '& .MuiChip-icon': { color: categoryColors[asset.assetClass] || '#6366f1' },
                                      '&:hover': { background: `${categoryColors[asset.assetClass] || '#6366f1'}20` },
                                    }}
                                  />
                                </Tooltip>
                              ))}
                            ...
                            <Stack direction="row" spacing={0.5} alignItems="center">
                              <Chip
                                ...
                                sx={{
                                  ...
                                  background: signal.confidence > 80 ? '#10b98120' : '#f59e0b20',
                                  color: signal.confidence > 80 ? '#10b981' : '#f59e0b',
                                }}
                              />
                              <Chip
